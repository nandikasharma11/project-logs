...
def test_app_retrieval_pipeline_execution():
    """Validates that get_retrieval_pipeline in app.py initializes and executes seamlessly
    with canonical DuckDB data, yielding citations, answer, and audit logging.
    """
    test_conn = duckdb.connect(":memory:")
    ...
    sample_df = pd.DataFrame([
        {
            "RecordID": "701",
            "TimeCreated": "2026-09-15 15:00:00",
            "EventID": "4625",
            "Level": "2",
            "LevelName": "Error",
            "Channel": "Security",
            ...
            "ThreadID": "100",
            "UserID": "TargetUser",
            "Message": "An account failed to log on. Subject: TargetUser.",
            "EventData": '{"TargetUserName": "TargetUser", "IpAddress": "10.0.0.99"}',
            "RawXML": "<Event>701</Event>",
        }
    ])

    app.sync_dataframe_to_duckdb(test_conn, sample_df, scope_key="pipeline_test_scope", force_resync=True)

    pipeline = app.get_retrieval_pipeline(backend_type="mock")
    assert pipeline is not None

    v_idx = app.get_vector_index(app.get_embedder())
    pipe_res = pipeline.execute(
        conn=test_conn,
        user_query="Show me failed logons for user TargetUser",
        vector_index=v_idx,
        canonical_table="canonical_logs",
    )

    assert pipe_res is not None
    assert len(pipe_res.retrieved_records) == 1
    assert pipe_res.answer != ""
    assert "[Record #701, Channel: Security]" in pipe_res.answer
    assert pipe_res.audit_metadata["model_name"] == "Qwen/Qwen2.5-7B-Instruct"

    # Verify audit trail written to DuckDB
    audit_rows = test_conn.execute("SELECT query_id, user_query, generated_answer FROM pipeline_audit_log").fetchall()
    assert len(audit_rows) >= 1
    assert audit_rows[0][0] == pipe_res.query_id

    # Verify response formatting with Qwen answer and audit metadata
    resp = app.generate_chatgpt_forensic_response(
        query="Show me failed logons for user TargetUser",
        qf=pipe_res.query_filter,
        records=pipe_res.retrieved_records,
        templates=pipe_res.matched_templates,
        scope_label="pipeline_test_scope",
        total_scope_records=1,
        conn=test_conn,
        qwen_answer=pipe_res.answer,
        audit_metadata=pipe_res.audit_metadata,
    )

    assert "#### 🤖 Qwen Cited Analysis" in resp
    assert "[Record #701, Channel: Security]" in resp
    assert "Generated by Qwen/Qwen2.5-7B-Instruct" in resp
