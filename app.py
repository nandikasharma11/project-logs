...
def generate_chatgpt_forensic_response(
    ...
    if qwen_answer:
        ...
        if audit_metadata:
            m_name = audit_metadata.get("model_name", "Qwen2.5")
            b_type = audit_metadata.get("backend", "local")
            lat = audit_metadata.get("llm_latency_ms", 0)
            resp += f"<div style='font-size: 0.78rem; color: #64748B; margin-top: -6px; margin-bottom: 14px;'>🛡️ <i>Generated by {m_name} ({b_type}) in {lat:.0f}ms • Verified against canonical records</i></div>\n\n"
